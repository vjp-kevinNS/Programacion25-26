...
public class Alumno {
    ...
    public void rellenarNotas() {
        Scanner entrada = new Scanner(System.in);
        // Mostramos el nombre del alumno que estamos editando
        System.out.println("Rellenando notas de: " + this.getNombre() + ": ");

        for (int i = 0; i < notas.length; i++) {
            // Mostramos el nombre de la asignatura específica
            System.out.print("Introduzca la nota de " + notas[i].getNombre() + ": ");

            // Guardamos la nota que introduce el usuario
            float notaIntroducida = entrada.nextFloat();
            notas[i].setNota(notaIntroducida);
        }
        System.out.println("Notas de " + this.getNombre() + " guardadas con éxito");
    }
    ...
    // Creamos To String 
    // Usamos Arrays.toString(notas) para que convierta el array en algo legible
    @Override
